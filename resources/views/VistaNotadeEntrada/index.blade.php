            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg"
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex flex-row items-center">
                        <h2 class="text-2xl font-semibold ml-4">Nota de Entrada</h2>
                        <a href="{{ route('notadeentrada.create') }}"
                            class="bg-indigo-600 text-white rounded-full px-4 py-2 text-base hover:bg-indigo-900 inline-flex items-center ml-auto">
                            Añadir
                        </a>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-300 mt-4">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($notadeentrada as $n)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-no-wrap">{{ $n->id }}</td>
                                        <td class="px-6 py-4 whitespace-no-wrap">{{ $n->users->name }}</td>
                                        <td class="px-6 py-4 whitespace-no-wrap">{{ $n->producto->nombre }}</td>
                                        <td class="px-6 py-4 whitespace-no-wrap">{{ $n->cantidad }}</td>
                                        <td class="px-6 py-4 whitespace-no-wrap">{{ $n->created_at }}</td>
