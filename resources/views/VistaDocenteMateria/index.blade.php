...
@section('content_header')
    <h1 class="text-2xl font-semibold">Maestro de Oferta</h1>
@stop
@section('content')
    <form class="w-full max-w-sm" action="{{ route('docentemateria.store') }}" method="POST">
        @csrf
        <div class="flex items-center border-b border-teal-500 py-2">
            <select
                class="appearance-none bg-transparent border-none w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none"
                name="docente_id" id="docente_id" aria-label="Full name">
            ...
            <select
                class="appearance-none bg-transparent border-none w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none"
                name="materia_id" id="materia_id" aria-label="Full name">
                ...
                <div class="text-gray-900 overflow-auto">
                    <table class="table-auto w-full divide-y divide-gray-300">
                        <div class="text-center mt-1 mb-2">
                            <a class="text-2x1 font-mono font-semibold text-black">MAESTRO DE OFERTA</a>
                        </div>
                        <thead>
                            <tr>
                        ...
                        @foreach ($docentemateria as $d)
                            <tbody>
                                <tr class="bg-white text-gray-700 hover:border-white hover:bg-gray-100 transition">
                                    ...
                                    <td class="py-3 text-sm text-left">{{ $d->docente->nombre }}</td>
                                    <td class="py-3 text-sm text-left">{{ $d->materia->nombre }}</td>
                                    ...
                                    <td class=" ">
                                        <div class="flex ml-4  justify-end text-right  ">
                                            ...
                                            <div class="flex justify-center">
                                                <a title="EDITAR" type="button" href="{{ route('docentemateria.edit', $d->id) }}"
                                            ...
                                            <div>
                                                <form action="{{ route('docentemateria.destroy', $d->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
